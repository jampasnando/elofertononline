            ...
            <div class="col-6 col-sm-4 col-md-3 col-lg-2">
                <div class="card product-card h-100">
                    ...
                    <div class="card-body d-flex flex-column p-2">
                        ...
                        <div class="mt-auto d-flex justify-content-between align-items-center">
                            <span class="fw-bold text-secondary small">Bs.{{$producto->precioventa}}</span>
                            <button class="btn btn-success btn-sm add-cart" data-id="{{ $producto->id }}" data-nombre="{{ $producto->descripcion }}" data-precio="{{ $producto->precioventa }}">Añadir</button>
                        </div>
                    </div>
                </div>
            </div>
...
@endsection
<script>
    // window.laravelData = @json(get_defined_vars()['__data'] ?? []);
    // console.log("📦 Datos recibidos en la vista:", window.laravelData);
</script>
